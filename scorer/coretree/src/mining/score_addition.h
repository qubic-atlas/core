...
struct ScoreAddition
{
    ...
    unsigned int computeScore(const unsigned char *publicKey, const unsigned char *nonce, const unsigned char *randomPool)
    {
        ...
        for (unsigned long long s = 0; s < numberOfMutations; ++s)
        {
            ...
            int accepted; unsigned int candidate;
            ...
            if (annxTraceCount < 4096) {
                ...
                e.step = s; e.bestScore = bestR; e.candidateScore = candidate;
                e.accepted = accepted; e.populationBefore = popBefore; e.populationAfter = currentANN.population;
            }
        }
        return bestR;
    }
