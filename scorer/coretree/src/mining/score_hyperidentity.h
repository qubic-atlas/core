...
struct ScoreHyperIdentity
{
    ...
    // --- ANN Explorer instrumentation (mutation trace) ---
    struct TraceEvent {
        unsigned long long step;
        unsigned int bestScore;
        unsigned int candidateScore;
        int accepted;                 // 1 accepted (kept), 0 rejected (rolled back)
        unsigned long long populationBefore;
        unsigned long long populationAfter;
    };
    TraceEvent annxTrace[4096];
    unsigned int annxTraceCount = 0;
    ...
    unsigned int computeScore(const unsigned char* publicKey, const unsigned char* nonce, const unsigned char* pRandom2Pool)
    {
        ...
        {
            unsigned long long popBefore = currentANN.population;
            ...
            // --- ANN Explorer instrumentation ---
            if (annxTraceCount < 4096) {
                TraceEvent& e = annxTrace[annxTraceCount++];
                e.step = s;
                e.bestScore = numberOfOutputNeurons - bestR;
                e.candidateScore = numberOfOutputNeurons - R;
                e.accepted = accepted;
                e.populationBefore = popBefore;
                e.populationAfter = currentANN.population;
            }
